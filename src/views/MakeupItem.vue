...
export default{
  ...
  methods: {
    viewProducts() {
        const path = "https://udm-backend.onrender.com/viewProduct?category=Makeup Accessories";
        axios.get(path)
        .then((res) => {
            if(res.data.length!=0){
    ...
    chosenProduct(product) {
        ...
        this.$router.push(`/makeup/${product[1]}`)
    },
    signout(){
...
<template>
  ...
  <div id="navigation">
    ...
    <RouterLink to='/makeup' target="_parent"><button id="activated2">Makeup Accessories</button></RouterLink>
  ...
  <div id="search">
    ...
    <ul v-if="selectedProduct" id="myMenu3">
        <li>
            ...
            <a :data="selectedProduct" v-bind:href="'mailto:'+sellerEmail+'?subject=UDM MAKEUP ACCESSORIES&body=Greetings, I am interested at the product with the name: '+selectedProduct[1]+'; that goes at Kshs. '+selectedProduct[5]+' with only '+selectedProduct[6]+' remaining. Kind regards, '+userFirstname+' '+userLastname+'.'"><input id="contact" type="submit" value="Email"></a>
        </li>
    </ul>  
  </div>
  <p id="warning">
