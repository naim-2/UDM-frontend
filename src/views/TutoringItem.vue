...
export default{
  ...
  methods: {
    viewProducts() {
        const path = "https://udm-backend.onrender.com/viewProduct?category=Tutoring Services";
        axios.get(path)
        .then((res) => {
            if(res.data.length!=0){
    ...
    chosenProduct(product) {
        ...
        this.$router.push(`/tutoring/${product[1]}`)
    },
    signout(){
...
<template>
  ...
  <div id="navigation">
    ...
    <RouterLink to='/tutoring' target="_parent"><button id="activated2">Tutoring Services</button></RouterLink>
