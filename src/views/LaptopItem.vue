...
export default{
  ...
  methods: {
    viewProducts() {
        const path = "https://udm-backend.onrender.com/viewProduct?category=Laptops";
        axios.get(path)
        .then((res) => {
            if(res.data.length!=0){
    ...
    chosenProduct(product) {
        ...
        this.$router.push(`/laptops/${product[1]}`)
    },
    signout(){
...
<template>
  ...
  <div id="navigation">
    ...
    <RouterLink to='/laptops' target="_parent"><button id="activated2">Laptops</button></RouterLink>
  ...
  <div id="search">
    ...
    <ul v-if="selectedProduct" id="myMenu3">
        <li>
            ...
            <a :data="selectedProduct" v-bind:href="'mailto:'+sellerEmail+'?subject=UDM LAPTOPS&body=Greetings, I am interested at the product with the name: '+selectedProduct[1]+'; that goes at Kshs. '+selectedProduct[5]+' with only '+selectedProduct[6]+' remaining. Kind regards, '+userFirstname+' '+userLastname+'.'"><input id="contact" type="submit" value="Email"></a>
        </li>
    </ul>  
  </div>
  <p id="warning">
