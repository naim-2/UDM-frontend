...
export default{
  ...
  methods: {
    ...
    verify(){
      ...
      const path = `https://udm-backend.onrender.com/filterProduct?category=Tutoring Services&minprice=${this.min}&maxprice=${this.max}`;
      axios.get(path)
      .then((res) => {
          if(res.data.length!=0){
...
<template>
  ...
  <div id="navigation">
    ...
    <button id="activated">Tutoring Services</button>
