...
export default{
  ...
  methods: {
    ...
    chosenProduct(product) {
        ...
        this.$router.push(`/laundry/${product[1]}`)
    },
    verify(){
      ...
      const path = `https://udm-backend.onrender.com/filterProduct?category=Laundry Services&minprice=${this.min}&maxprice=${this.max}`;
      axios.get(path)
      .then((res) => {
          if(res.data.length!=0){
...
<template>
  ...
  <div id="navigation">
    ...
    <button id="activated">Laundry Services</button>
